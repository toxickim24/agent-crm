...
const Mailers = () => {
  ...
  const handleAddContact = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/mailers/add-contact`, {
  ...
  // Filter contacts not in mailers for add contact modal (use lead_id)
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 dark:text-yellow-400 mt-1">
                The DealMachine API is not configured. Please contact your administrator to set up the integration with bearer token and API endpoints.
              </p>
            </div>
          </div>
        </div>
                ...
                >
                  ...
                  {availableContacts.map((contact) => (
                    <option key={contact.id} value={contact.id}>
                      {contact.contact_first_name} {contact.contact_last_name} - Lead ID: {contact.lead_id}
                    </option>
                  ))}
                </select>
              </div>
